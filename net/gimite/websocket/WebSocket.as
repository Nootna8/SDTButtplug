...
public class WebSocket extends EventDispatcher {
  ...
  public function close(
    ...
    if (code != STATUS_NORMAL_CLOSURE &&
      ...
      logger.error(StringUtil.substitute(
          "Fail connection by {origin}: code={code} reason={reason}", {"origin": origin, "code": code, "reason": reason}));
    }
  ...
  private function onSocketConnect(event:Event):void {
    ...
    var req:String = StringUtil.substitute(
      ...
      "{opt}" +
      "\r\n",
      {"path": path, "host": hostValue, "key": key, "origin": origin, "cookie": cookie, "opt": opt});
  ...
  private function generateKey():String {
	...
	var encoded:String = Base64.encodeByteArray(vals);
